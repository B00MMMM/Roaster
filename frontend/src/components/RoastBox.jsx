...
export default function RoastBox() {
  ...
  // Scroll when typing starts (when displayed text changes from empty to having content)
  useEffect(() => {
    if (typedRoast && typedRoast.length > 0 && roastResultRef.current) {
  ...
  useEffect(() => {
    if (typewriterComplete && showRoast) {
      // Final scroll to bottom when typewriter completes (keeping this as backup)
      setTimeout(() => {
        window.scrollTo({
          top: document.body.scrollHeight,
          behavior: 'smooth'
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 relative overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-4 py-8 max-w-5xl mt-5">
        
        {/* Desktop Layout (lg and above) */}
        <div className="hidden lg:block">
          {/* Header */}
          <div className="text-center mb-20 animate-fade-in lg:scale-90 xl:scale-85">
            <div className="flex flex-wrap items-center justify-center gap-2 mb-4">
              <span className="text-orange-500 text-3xl animate-bounce">🔥</span>
              <h1 className="text-3xl lg:text-4xl xl:text-5xl font-extrabold bg-gradient-to-r from-orange-400 via-red-500 to-yellow-500 bg-clip-text text-transparent animate-gradient-x">
                RoastMe
              </h1>
              <span className="text-orange-500 text-3xl animate-bounce delay-200">🔥</span>
            </div>
            <p className="text-base lg:text-lg text-slate-300 font-medium animate-slide-up">
              Prepare to get absolutely <span className="text-orange-400 font-bold">ROASTED</span> 🔥
            </p>
            <p className="text-sm lg:text-base text-slate-400 mt-2 animate-slide-up delay-200">
              Spin the wheel and let fate decide your roast level!
            </p>
          </div>

          {/* Spinning Wheel + Button Row */}
          <div className="animate-slide-up delay-300 mb-12 lg:scale-90 xl:scale-85">
            <div className="flex items-center justify-center gap-4 mb-4">
              {/* Spinning Wheel - Smaller Size for Desktop */}
              <div className="w-48 lg:w-56 h-48 lg:h-56 flex items-center justify-center">
                <SpinningWheel
              ...
              {/* Button - Smaller Size for Desktop */}
              <button
                onClick={handleRoast}
                disabled={loading}
                className="w-48 h-48 lg:h-48 text-base font-bold bg-gradient-to-r from-orange-600 to-red-600 hover:from-orange-700 hover:to-red-700 text-white ml-8 mb-8 rounded-lg shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-300 disabled:opacity-70 disabled:cursor-not-allowed disabled:hover:scale-100 flex flex-col items-center justify-center gap-2"
              >
                {loading ? (
                  <div className="flex flex-col items-center gap-2">
                    <span className="animate-spin text-xl">⚡</span>
                    <span className="text-center text-sm">Spinning<br />Wheel...</span>
                  </div>
                ) : (
                  <div className="flex flex-col items-center gap-2">
                    
                    <span className="text-center font-bold text-sm"><span className="text-2xl">🔥</span>ROAST ME!<span className="text-2xl">🔥</span></span>
                  </div>
                )}
              </button>
            </div>
          ...
          {/* Input Section */}
          <div className="flex justify-center mb-8 w-full px-2 sm:px-0 lg:scale-90 xl:scale-85">
        ...
        <div className="lg:hidden">
          ...
          {/* Input + Button */}
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-6 w-full px-2 sm:px-0">
